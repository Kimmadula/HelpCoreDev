...
export default function SubsectionPages({ subsectionId }) {
    ...
    const loadPages = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`/api/admin/subsections/${subsectionId}/pages`);
            setPages(res.data);
        } finally {
            setLoading(false);
        }
    ...
    const createPage = async (e) => {
        e.preventDefault();
        try {
            await axios.post(`/api/admin/subsections/${subsectionId}/pages`, {
                title,
    ...
    return (
        <AuthenticatedLayout header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Admin • Subsection pages</h2>}>
            <Head title="Admin Subsection Pages" />
            ...
            <div className="py-8">
                <div className="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    <div className="flex items-center justify-between">
                        <button
                            onClick={(e) => {
                                ...
                                else window.location.href = "/admin/products"; // Fallback
                            }}
                            className="text-sm underline hover:text-black transition text-gray-600 inline-block"
                        >
                            ← Back
                        </button>
                    </div>
                    ...
                    <div className="bg-white shadow rounded-lg p-6">
                        <h3 className="font-semibold mb-4">Add Page</h3>
                                ...
                                {pages.map((p, idx) => (
                                    <div key={p.id} className="py-3 flex items-center justify-between gap-3">
                                        ...
                                        <div className="flex gap-2 shrink-0">
                                            ...
                                            <a className="border rounded px-3 py-1 text-sm" href={`/admin/pages/${p.id}/blocks`}>
                                                Edit Content
                                            </a>
                                        </div>
                                    </div>
